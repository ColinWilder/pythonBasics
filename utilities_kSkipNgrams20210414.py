...
def makeKSkipBigrams(inputWordList, wordDistance):
	# wordDistance should be the k skip level i.e. how many words can be skipped
	# arguments: the string and the number of words in the n-grams 
	# takes string and returns list of bigrams in it
	outputKSkipBigramsList=[] # the final result
	for i in range(0,len(inputWordList)-1):
		outerLimit=min(len(inputWordList),i+wordDistance+2)
		for j in range(i+1,outerLimit):
			# print(inputWordList[i]+" "+inputWordList[j])
			outputKSkipBigramsList.append((inputWordList[i], inputWordList[j])) # add a bigram to outputBigramsList
	return outputKSkipBigramsList

def countNgram(kSkipBigram, ngramDictionary):
# takes a list of k-skip-bigrams as argument
# returns a dictionary with bigrams as keys, their frequencies as values
    pairTuple=kSkipBigram[0], kSkipBigram[1]
...
# TO DO: Change name of this function to something like "addTokenToTokenDictionaryOnce".
